...
from jyw_login import LoginClass
...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        self.user.setText('')   # 유저 로그인 상태 표시 라벨 공백 처리
        self.id = False         # 로그인 없이 예약창 들어 가지 않게 하기 위해 False 선언
        self.column = 5         # 테이블 위젯 열수

    def reserve(self):  # 예약창 불러 오기
        if self.id:
            self.res = Reserve()
